...
import type { ChannelCreate, BackfillMode } from '@/types/channel'
import { AxiosError } from 'axios'

type BackfillOption = BackfillMode | 'NONE'
...
export function AddChannelModal({
  ...
  const [backfillMode, setBackfillMode] = useState<BackfillOption>('LAST_N_MESSAGES')
  const [backfillValue, setBackfillValue] = useState(100)
  const [startBackfill, setStartBackfill] = useState(true)
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setBackfillMode('LAST_N_MESSAGES')
      setBackfillValue(100)
      setStartBackfill(true)
    }
  }, [isOpen])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const channelData: ChannelCreate = {
      ...
      // Backfill settings
      backfill_mode: backfillMode === 'NONE' ? undefined : backfillMode,
      backfill_value: backfillMode === 'NONE' ? undefined : backfillValue,
      start_backfill: backfillMode !== 'NONE' && startBackfill,
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary rounded-lg shadow-xl w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4 min-h-[280px]">
          ...
          {/* Backfill Options (collapsible) */}
          {resolvedChannel && (
            <div className="border border-bg-tertiary rounded-lg overflow-hidden">
              <button
                type="button"
                onClick={() => setShowAdvanced(!showAdvanced)}
                className="w-full px-4 py-3 flex items-center justify-between text-left text-text-secondary hover:bg-bg-tertiary/50 transition-colors"
              >
                <span className="text-sm font-medium">Backfill Options</span>
                <ChevronIcon className={`w-4 h-4 transition-transform ${showAdvanced ? 'rotate-180' : ''}`} />
              </button>

              {showAdvanced && (
                <div className="px-4 pb-4 pt-2 space-y-4 border-t border-bg-tertiary">
                  {/* Backfill Mode */}
                  <div>
                    <label
                      htmlFor="backfillMode"
                      className="block text-sm font-medium text-text-secondary mb-1"
                    >
                      Backfill Mode
                    </label>
                    <select
                      id="backfillMode"
                      value={backfillMode}
                      onChange={(e) => setBackfillMode(e.target.value as BackfillOption)}
                    ...
                    >
                      ...
                      <option value="NONE">No backfill</option>
                    </select>
                    <p className="mt-1 text-xs text-text-muted">
                      {backfillMode === 'LAST_N_MESSAGES' && 'Fetch the most recent N messages from this channel'}
                      {backfillMode === 'LAST_N_DAYS' && 'Fetch messages from the last N days'}
                      {backfillMode === 'ALL_HISTORY' && 'Fetch all available messages (may take a while)'}
                      {backfillMode === 'NONE' && 'Skip backfill, only monitor new messages'}
                    </p>
                  </div>

                  {/* Backfill Value (only for LAST_N_MESSAGES and LAST_N_DAYS) */}
                  {(backfillMode === 'LAST_N_MESSAGES' || backfillMode === 'LAST_N_DAYS') && (
                    <div>
                      <label
                        htmlFor="backfillValue"
                        className="block text-sm font-medium text-text-secondary mb-1"
                      >
                      ...
                      <input
                        ...
                        type="number"
                        min={1}
                        max={backfillMode === 'LAST_N_MESSAGES' ? 10000 : 365}
                        value={backfillValue}
                        onChange={(e) => setBackfillValue(Math.max(1, parseInt(e.target.value) || 1))}
                        className="w-full px-3 py-2 bg-bg-tertiary border border-bg-tertiary rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-accent-primary"
                      />
                    </div>
                  )}
                  ...
                  {backfillMode !== 'NONE' && (
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={startBackfill}
                        onChange={(e) => setStartBackfill(e.target.checked)}
                        className="w-4 h-4 rounded border-bg-tertiary bg-bg-tertiary text-accent-primary focus:ring-accent-primary focus:ring-offset-0"
                      />
                      <span className="text-sm text-text-secondary">Start backfill immediately</span>
                    </label>
                  )}
                </div>
              )}
